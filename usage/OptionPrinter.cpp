...
namespace myleft {
	...
	std::string OptionPrinter::usage() {
		...
		usageDesc << "[";
		for (std::vector<myleft::CustomOptionDescription>::iterator it = options_.begin();
				it != options_.end();
				++it) {
		...
		/** positional option
		 */
		for (std::vector<myleft::CustomOptionDescription>::iterator it = positionalOptions_.begin();
	...
	void OptionPrinter::printStandardAppDesc(const std::string& appName,
		...
		myleft::OptionPrinter optionPrinter;
		...
		for (Options::iterator it = allOptions.begin();
				it != allOptions.end();
				++it) {
			myleft::CustomOptionDescription currOption(*it);
			if (positionalDesc) {
